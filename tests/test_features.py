...
class TestAddEngineeredFeatures:
    ...
    def test_score_state_mutually_exclusive(self):
        ...
        state_sum = df_feat["is_trailing"] + df_feat["is_tied"] + df_feat["is_leading"]
...
class TestBuildFeatureMatrix:
    ...
    def test_uses_custom_target_col_when_specified(self):
        ...
        X, y, feature_cols = features.build_feature_matrix(
            df, target_col="custom_target"
        )
    ...
    def test_raises_when_target_col_missing(self):
        ...
        assert "target" in str(exc_info.value).lower() or "is_pass" in str(
            exc_info.value
        )
    ...
    def test_raises_when_feature_cols_missing_after_engineering(self, monkeypatch):
        ...
        fake_features = CONFIG.features.engineered_feature_candidates + (
            "fake_feature",
        )
        monkeypatch.setattr(
            CONFIG.features,
